...
const SUPPORTED_LOCALES = ['ro', 'en', 'fr', 'it', 'de', 'es', 'uk', 'hu'] as const
const REFERENCE_LOCALES = ['ro', 'en'] as const
 ...
 */
function getNestedValue(obj: Translations, keyPath: string): any {
 ...
 */
function setNestedValue(obj: Translations, keyPath: string, value: any): void {
  const keys = keyPath.split('.')
  let current: any = obj
 ...
 */
function isAutoTranslated(value: string): boolean {
 ...
 */
function loadTranslations(locale: string): Translations | null {
  try {
 ...
 */
function saveTranslations(locale: string, translations: Translations): void {
  const filePath = path.join(TRANSLATIONS_DIR, `${locale}.json`)
 ...
 * Main sync function
 */
function syncAllTranslations(): void {
